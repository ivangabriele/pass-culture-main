...
Compares PostgreSQL vs TimescaleDB implementations.

Usage:
    # From workspace root, with venv activated:
    cd api
    python ../scripts/benchmark_bookings_query.py --database postgres --output results_postgres.json
    python ../scripts/benchmark_bookings_query.py --database timescaledb --output results_timescaledb.json
...
class BookingQueryBenchmark:
    ...
    def get_disk_sizes(self) -> dict[str, Any]:
        """Get disk usage statistics for booking table and indexes."""
        stats = {
            "table_size_bytes": 0,
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                """)
                is_timescaledb = cursor.fetchone()[0]
                ...
                if is_timescaledb:
                    ...
                    if comp_result and comp_result[0]:
                        stats["compression_stats"] = {
                            ...
                            "compression_ratio": float(comp_result[4]) if comp_result[4] else None,
                        }
    ...
    def find_test_entities(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            if result:
                ...
                logger.error("No suitable test entities found! Database may not have enough data.")
                sys.exit(1)
    ...
    def benchmark_list_query(
        ...
        logger.info(f"Benchmarking list query (last {period_days} days, page {page})...")
        ...
        metrics = self.run_query_with_metrics(
            query,
            (self.test_user_id, start_date, end_date, self.test_offerer_id, per_page, offset),
            fetch_results=True,
        )
    ...
    def benchmark_status_filter_query(self, status: str, period_days: int = 90) -> PerformanceMetrics:
        """Benchmark query with status filter."""
        logger.info(f"Benchmarking status filter query (status={status}, last {period_days} days)...")
    ...
    def run_full_benchmark_suite(self) -> list[PerformanceMetrics]:
        ...
        test_scenarios = [
            ...
            ("status_filter", lambda: self.benchmark_status_filter_query("CONFIRMED", 90)),
            ("status_filter", lambda: self.benchmark_status_filter_query("USED", 90)),
            ("status_filter", lambda: self.benchmark_status_filter_query("CANCELLED", 90)),
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark booking queries performance")
    parser.add_argument(
        "--database",
    ...
    parser.add_argument("--port", type=int, help="Database port (auto-detected if not specified)")
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of runs for each query (default: 10)",
    )

    args = parser.parse_args()

    if args.port is None:
        args.port = 5434 if args.database == "postgres" else 5435

    logger.info("=" * 70)
    logger.info(f"Booking Queries Performance Benchmark")
    ...
    for run in range(1, args.runs + 1):
        ...
        try:
            ...
            if run == 1:
                ...
                logger.info(f"Disk usage: {disk_sizes['total_size_pretty']} "
                ...
                if disk_sizes["compression_stats"]:
                    ...
                    logger.info(f"Compression: {comp['compressed_chunks']}/{comp['total_chunks']} chunks compressed, "
            ...
            for result in results:
                ...
                result_dict["run"] = run
                result_dict["database"] = args.database
