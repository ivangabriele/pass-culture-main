...
from tests.routes.public.helpers import PublicAPIRestrictedEnvHelper
...
def payload_fixture(redactor, institution):
    return {"uai": institution.institutionId, "redactor_id": redactor.id}
...
class ConfirmCollectiveBookingTest(PublicAPIRestrictedEnvHelper):
    endpoint_url = "/v2/collective/bookings/{booking_id}/confirm"
    ...
    def test_confirm_pending_booking(self, client, payload, pending_booking):
        with assert_status_changes_to(pending_booking, models.CollectiveBookingStatus.CONFIRMED):
            self.confirm_booking(client, payload, pending_booking.id, status_code=204)

    def test_confirm_confirmed_booking(self, client, payload, confirmed_booking):
        with assert_status_does_not_change(confirmed_booking):
            self.confirm_booking(client, payload, confirmed_booking.id, status_code=204)

    def test_confirm_used_booking(self, client, payload, used_booking):
        error = {"code": "CONFIRMATION_LIMIT_DATE_HAS_PASSED"}

        with assert_status_does_not_change(used_booking):
            self.confirm_booking(client, payload, used_booking.id, status_code=403, json_error=error)

    def test_confirm_reimbursed_booking(self, client, payload, reimbursed_booking):
        error = {"code": "CONFIRMATION_LIMIT_DATE_HAS_PASSED"}

        with assert_status_does_not_change(reimbursed_booking):
            self.confirm_booking(client, payload, reimbursed_booking.id, status_code=403, json_error=error)

    def test_confirm_cancelled_booking(self, client, payload, cancelled_booking):
        error = {"code": "EDUCATIONAL_BOOKING_IS_CANCELLED"}

        with assert_status_does_not_change(cancelled_booking):
            self.confirm_booking(client, payload, cancelled_booking.id, status_code=403, json_error=error)

    def test_confirm_when_insufficient_fund(self, client, payload, institution, pending_booking):
        for deposit in institution.deposits:
            deposit.amount = 0

        error = {"code": "INSUFFICIENT_FUND"}
        with assert_status_does_not_change(pending_booking):
            self.confirm_booking(client, payload, pending_booking.id, status_code=403, json_error=error)

    @override_features(ENABLE_EAC_FINANCIAL_PROTECTION=True)
    def test_confirm_when_insufficient_ministry_fund(self, client, payload, used_booking, pending_booking):
    ...
    def test_confirm_when_insufficient_temporary_fund(self, client, payload, institution, pending_booking):
        for deposit in institution.deposits:
            deposit.amount = 0
    ...
    def test_confirm_unknown_booking(self, client, payload):
        error = {"code": constants.EDUCATIONAL_BOOKING_NOT_FOUND}
        self.confirm_booking(client, payload, 0, status_code=404, json_error=error)

    def test_confirm_unknown_deposit(self, client, payload, institution, pending_booking):
        for deposit in institution.deposits:
            deposit.educationalYear = factories.EducationalYearFactory()

        error = {"code": "DEPOSIT_NOT_FOUND"}
        with assert_status_does_not_change(pending_booking):
            self.confirm_booking(client, payload, pending_booking.id, status_code=404, json_error=error)

    def confirm_booking(self, client, payload, booking_id, status_code, json_error=None):
        response = self.send_request(
            self.get_authenticated_client(client), url_params={"booking_id": booking_id}, json=payload
        )
