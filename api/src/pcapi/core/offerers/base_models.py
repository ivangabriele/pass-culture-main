...
class Venue(PcObject, Base, Model, HasThumbMixin, AccessibilityMixin):
    ...
    managingOfferer: sa_orm.Mapped["offerers_models.Offerer"] = sa_orm.relationship(
    ...
    venueLabel: sa_orm.Mapped["offerers_models.VenueLabel"] = sa_orm.relationship(
        "VenueLabel", foreign_keys=[venueLabelId]
    )
    ...
    contact: sa_orm.Mapped["offerers_models.VenueContact | None"] = sa_orm.relationship(
    ...
    googlePlacesInfo: sa_orm.Mapped["offerers_models.GooglePlacesInfo | None"] = sa_orm.relationship(
    ...
    venueEducationalStatus: sa_orm.Mapped["offerers_models.VenueEducationalStatus"] = sa_orm.relationship(
    ...
    collectiveStudents: sa_orm.Mapped[list["educational_models.StudentLevels"]] | None = sa.Column(
    ...
    registration: sa_orm.Mapped["offerers_models.VenueRegistration | None"] = sa_orm.relationship(
        "VenueRegistration", back_populates="venue", uselist=False
    )

    bankAccountLinks: list["offerers_models.VenueBankAccountLink"] = sa_orm.relationship(
        "VenueBankAccountLink", back_populates="venue", passive_deletes=True
    )

    accessibilityProvider: sa_orm.Mapped["offerers_models.AccessibilityProvider | None"] = sa_orm.relationship(
    ...
    offererAddress: sa_orm.Mapped["offerers_models.OffererAddress | None"] = sa_orm.relationship(
    ...
    @property
    def current_bank_account_link(self) -> "offerers_models.VenueBankAccountLink | None":
        now = datetime.datetime.utcnow()
    ...
    @property
    def confidenceLevel(self) -> "offerers_models.OffererConfidenceLevel | None":
